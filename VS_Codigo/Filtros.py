    ...
    def run(self):
        ...
        while not self.stop_evt.is_set():
            try:
                ...
                # Encola datos para el hilo principal (constelación)
                app.enqueue_syms(rx_syms)
                # Métricas: pásalas también vía after para seguridad
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("PlutoSDR QPSK OTA • Filtros + Constelación (thread-safe)")
    ...
    def _build_ui(self):
        ...
        mid = ttk.LabelFrame(self, text="Constelación (QPSK)")
        mid.pack(fill="both", expand=True, padx=10, pady=8)
        self.fig = Figure(figsize=(5, 4), dpi=100)
        self.ax = self.fig.add_subplot(111)
        self.ax.grid(True)
        self.ax.set_aspect("equal", adjustable="box")
        self.ax.set_xlim(-2, 2); self.ax.set_ylim(-2, 2)
        self.ax.set_xlabel("I"); self.ax.set_ylabel("Q")
        # Scatter persistente + puntos ideales
        self.scatter = self.ax.scatter([], [], s=8, alpha=0.6)
        ref = (np.array([1+1j, -1+1j, -1-1j, 1-1j]) / np.sqrt(2)).astype(np.complex64)
        self.ax.scatter(ref.real, ref.imag, marker="x", s=80)
        self.canvas = FigureCanvasTkAgg(self.fig, master=mid)
        self.canvas.get_tk_widget().pack(fill="both", expand=True)

        right = ttk.LabelFrame(self, text="Métricas")
        right.pack(fill="x", padx=10, pady=4)
        self.lbl_metrics = ttk.Label(right, text="EVM: -- %    CFO: -- kHz    SPS: --", font=("Segoe UI", 11))
    ...
    def enqueue_syms(self, syms):
        try:
            while not self.syms_queue.empty():
                self.syms_queue.get_nowait()
        except Exception:
            pass
        self.syms_queue.put(syms[:2000].copy())

    def _ui_tick(self):
        try:
            if not self.syms_queue.empty():
                syms = self.syms_queue.get_nowait()
            ...
            self.ui_timer_id = None
            return
        self.ui_timer_id = self.after(60, self._ui_tick)
    ...
    def start_txrx(self):
        ...
        try:
            ...
            self.log("TX QPSK iniciado (cíclico). RX activo. Observa la constelación.")
